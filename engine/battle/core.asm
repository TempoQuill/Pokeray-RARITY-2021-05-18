...
; Roaming monsters work differently
...
; Species-specfic:
; if hi byte is active, we can skip to the next step
; without this check
; pokemon with the lo byte value $c9 would be treated as if it was Unown
	ld a, [wTempEnemyMonSpecies + 1]
	and a
	jr nz, .Happiness
...
.Happiness:
; Set happiness
	ld a, BASE_HAPPINESS ; aka BASE_QUALITY_OF_LIFE
	ld [wEnemyMonHappiness], a
; Set quality of life
	ld [wEnemyMonQualityOfLife], a
; Set level
	ld a, [wCurPartyLevel]
	ld [wEnemyMonLevel], a
; Fill stats
	...
	ld b, FALSE
	ld hl, wEnemyMonDVs - (MON_DVS - MON_STAT_EV + 1)
	predef CalcMonStats
...
; If we're headbutting trees, some monsters enter battle asleep
	...
	and a
	ld a, [wTempEnemyMonSpecies]
	jr z, .dismiss_new_mons

; Croven/Astroven are asleep if MORN/DAY
; Croven/Astroven's index no is $10a/$10b
	cp LOW(CROVEN)
	jr z, .check_high_byte
	cp LOW(ASTROVEN)
	jr z, .check_high_byte
	jr .not_sleeping
...
.check_high_byte
; at this point the high byte is guarateed to be active
; so this can never indicate CATERPIE or METAPOD
; does the high byte's value indicate EQUINTO_MONS?
	ld a, [wTempEnemyMonSpecies + 1]
; it could be 1 out of 508 possible glitch mons
	cp EQUINTO_MONS
	jr nz, .not_sleeping
	; yes, so it's Croven/Astroven
...
; in this particular case, pokerus removes one of the rotation instructions
; this effectively doubles ev distribution
	and BASE_EV_MASK
	swap a
	rra
	ld b, a
...
GetRoamMonMapGroup:
	...
	cp b
	jr z, .compare_hi_byte_1
.next_check
	ld a, [wRoamMon2Species]
	cp b
	jr z, .compare_hi_byte_2
.just_load
