...
EMail_FindOpenSlot:
	call GetRemainingSpaceInEmailList
	ld b, a
	ld hl, wEMailList
.loop
	ld a, [hli]
...
GetRemainingSpaceInEmailList:
	xor a
	ld [wBuffer1], a
	ld hl, PermanentAddresses
.loop
	ld a, [hli]
	cp -1
	jr z, .done
...
.loop
	ld [wEMailListIndex], a
	ld a, [de]
	and a
	jr z, .not_good_for_call
	ld hl, PhoneContacts + EMAIL_CONTACT_SCRIPT2_TIME
	ld bc, EMAIL_CONTACT_SIZE
	call AddNTimes
	ld a, [wCheckedTime]
	and [hl]
	jr z, .not_good_for_call
	ld bc, EMAIL_CONTACT_MAP_GROUP - EMAIL_CONTACT_SCRIPT2_TIME
	add hl, bc
	ld a, [wMapGroup]
	cp [hl]
	jr nz, .different_map
...
CheckSpecialPhoneCall::
	...
	call LoadCallerScript
	pop hl
	ld de, wCallerContact + EMAIL_CONTACT_SCRIPT2_BANK
	ld a, [hli]
	ld [de], a
	inc de
	ld a, [hli]
...
Function901a1:
	...
	ld d, h
	ld e, l
	ld hl, EMAIL_CONTACT_SCRIPT1_TIME
	add hl, de
	ld a, [hl]
	...
	ld hl, EMAIL_CONTACT_MAP_GROUP
	add hl, de
	ld a, [wMapGroup]
	cp [hl]
	jr nz, .GetPhoneScript
	ld hl, EMAIL_CONTACT_MAP_NUMBER
	add hl, de
	ld a, [wMapNumber]
	cp [hl]
	jr nz, .GetPhoneScript
...
.GetPhoneScript:
	ld hl, EMAIL_CONTACT_SCRIPT1_BANK
	add hl, de
	ld b, [hl]
	ld hl, EMAIL_CONTACT_SCRIPT1_ADDR
	add hl, de
	ld a, [hli]
	ld h, [hl]
	ld l, a
...
Script_ReceivePhoneCall:
	...
	memcall wCallerContact + EMAIL_CONTACT_SCRIPT2_BANK
	waitbutton
	callasm HangUp
	closetext
...
GetCallerTrainerClass:
	push hl
	ld hl, PhoneContacts + EMAIL_CONTACT_TRAINER_CLASS
	ld bc, EMAIL_CONTACT_SIZE
	call AddNTimes
	ld a, [hli]
	ld b, [hl]
...
GetCallerLocation:
	...
	push de
	ld a, [wCurCaller]
	ld hl, PhoneContacts + EMAIL_CONTACT_MAP_GROUP
	ld bc, EMAIL_CONTACT_SIZE
	call AddNTimes
	ld b, [hl]
	inc hl
