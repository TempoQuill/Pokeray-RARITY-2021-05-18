...
; entries correspond to item ids
	...
	dw StatusHealingEffect ; PARALYZEHEAL
	...
	dw StatusHealingEffect ; SHINY_BERRY
	dw StatusHealingEffect ; SPICY_BERRY
	dw StatusHealingEffect ; FROSTY_BERRY
	...
	dw NoEffect            ; MAGNET
	dw StatusHealingEffect ; AUX_BERRY
	dw NoEffect            ; PEARL
	dw NoEffect            ; SILVER_PEARL
...
LoveBallMultiplier:
; multiply catch rate by 8 if mons are of same species, different gender
...
GetStatEVRelativePointer:
	ld a, [wCurItem]
	ld hl, StatEvItemPointerOffsets
.next
	cp [hl]
	inc hl
	jr z, .got_it
...
StatEvItemPointerOffsets:
	db HP_UP,    MON_HP_EV - MON_STAT_EV
	db PROTEIN, MON_ATK_EV - MON_STAT_EV
	db IRON,    MON_DEF_EV - MON_STAT_EV
	db CARBOS,  MON_SPD_EV - MON_STAT_EV
	db CALCIUM, MON_SPC_EV - MON_STAT_EV
