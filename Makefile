roms := pokeray.gbc pokeshade.gbc
...
ray_excl_obj := $(addsuffix _ray.o,$(rs_excl_asm))
shade_excl_obj := $(addsuffix _shade.o,$(rs_excl_asm))
...
#	rm -f $(roms) $(pokegold_obj) $(pokesilver_obj) $(pokegold_debug_obj) $(pokesilver_debug_obj) $(roms:.gbc=.map) $(roms:.gbc=.sym) rgbdscheck.o
	rm -f $(roms) $(pokeray_obj) $(pokeshade_obj) $(roms:.gbc=.map) $(roms:.gbc=.sym) rgbdscheck.o
	$(MAKE) clean -C tools/
...
$(pokeray_obj):   RGBASMFLAGS += -D _RAY
$(pokeshade_obj): RGBASMFLAGS += -D _SHADE
...
pokeray_opt   = -cjsv -t POKEMON_RAY -i ATUE -k 01 -l 0x33 -m 0x10 -r 5 -p 0
pokeshade_opt = -cjsv -t POKEMON_SHD -i ATXE -k 01 -l 0x33 -m 0x10 -r 5 -p 0
